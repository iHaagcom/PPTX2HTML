...
class App(QWidget):
    def __init__(self):
        super().__init__()
        self.title = 'PowerPoint to HTML Converter - Developed by Trevor Haagsma'
        self.left = 100
        self.top = 100
        self.width = 600
        self.height = 400
    ...
    def initUI(self):
        ...
        self.export_location_combo = QComboBox()
        self.export_location_combo.setEditable(True)
        self.populate_export_locations()
        self.export_location_combo.currentTextChanged.connect(self.update_info_label)

        self.export_button = QPushButton('Browse')
        self.export_button.clicked.connect(self.select_export_location)
        self.export_button.setMaximumWidth(66)  # Make the button smaller
        
        export_path_layout.addWidget(self.export_location_combo, 4)  # Give more space to the combo box
        export_path_layout.addWidget(self.export_button, 1)  # Give less space to the button
        ...
        self.update_info_label(self.export_location_combo.currentText())
    ...
    def populate_export_locations(self):
        model = QStandardItemModel()
        for category, locations in EXPORT_LOCATIONS.items():
        ...
        self.export_location_combo.setModel(model)

    def save_settings(self):
        settings = {
            "export_location": os.path.normpath(self.export_location_combo.currentText()),
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists(self.settings_file):
                ...
                if hasattr(self, 'export_location_combo'):
                    export_location = os.path.normpath(settings.get("export_location", ""))
                    index = self.export_location_combo.findText(export_location)
                    if index >= 0:
                        self.export_location_combo.setCurrentIndex(index)
                    else:
                        self.export_location_combo.setCurrentText(export_location)
                    self.update_info_label(export_location)
                    logging.info(f"Export location set to: {export_location}")
    ...
    def update_info_label(self, location):
        try:
            normalized_location = os.path.normpath(location)
            info = None
            for category, locations in EXPORT_LOCATIONS.items():
                if normalized_location in locations:
                    info = locations[normalized_location]
                    break

            if info is None:  # This is a custom location
                self.info_label.setText(f"""
                    <p style='margin: 0;'>
                        Location Type: <span style='color: grey;'>Custom</span><br>
                        <span style='color: green;'>Custom</span>
                        <span style='color: purple;'>Custom</span>
                    </p>
                """)
                logging.info(f"Custom location selected: {normalized_location}")
            else:
                location_type, description, additional_info = info + ("", "", "")[:3 - len(info)]
                self.info_label.setText(f"""
                    <p style='margin: 0;'>
                        Location Type: <span style='color: grey;'>{location_type}</span>
                        <span style='color: green;'>{description}</span> 
                        <span style='color: purple;'>{additional_info}</span>
                    </p>
                """)
                logging.info(f"Predefined location selected: {normalized_location}")

        except Exception as e:
            logging.error(f"Error updating info label: {str(e)}", exc_info=True)
            self.info_label.setText("Error updating location information")


    def select_export_location(self):
        logging.info("Browse button clicked, starting directory selection...")
        try:
            logging.info("Selecting export location...")
            ...
            if export_dir:
                ...
                self.export_location_combo.setCurrentText(export_dir)
                self.update_info_label(export_dir)
                logging.info(f"Export location selected: {export_dir}")
                self.save_current_settings()
        except Exception as e:
            logging.error(f"Error selecting export location: {str(e)}", exc_info=True)
            QMessageBox.critical(self, "Error", f"An error occurred while selecting the export location:\n{str(e)}")
    ...
    def start_conversion(self):
        try:
            ...
            use_whisper = self.whisper_checkbox.isChecked()
            logging.info(f"Use Whisper: {use_whisper}")
            slide_duration = self.duration_slider.value()
            export_location = self.export_location_combo.currentText()
            
            logging.info(f"Starting conversion with parameters:")
            logging.info(f"Template file: {self.template_file}")
            logging.info(f"Export location: {export_location}")
            logging.info(f"Open in browser: {open_in_browser}")
            logging.info(f"Use Whisper: {use_whisper}")
            logging.info(f"Slide duration: {slide_duration}")
            
            self.worker = WorkerThread(self.template_file, export_location, open_in_browser, use_whisper, slide_duration)
            ...
            logging.error(f"Error in start_conversion: {str(e)}", exc_info=True)
    ...
    def select_export_location(self):
        ...
        try:
            ...
            if folder:
                ...
                logging.info(f"Selected export directory: {folder}")
                self.export_location_combo.setCurrentText(folder)
                self.update_info_label(folder)
                logging.info(f"Export location selected: {folder}")
                self.save_current_settings()
        except Exception as e:
            logging.error(f"Error selecting export location: {str(e)}", exc_info=True)
            QMessageBox.critical(self, "Error", f"An error occurred while selecting the export location:\n{str(e)}")
